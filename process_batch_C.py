...
def convert_k_to_f(temp_k):
    """Convert Kelvin to Fahrenheit and round to nearest whole number."""
    logging.debug(f"Calling convert_k_to_f() with {temp_k}.")
    fahrenheit = round((temp_k - 273.15) * 9/5 + 32)
    logging.debug(f"Converted {temp_k}K to {fahrenheit}F.")
    return fahrenheit

def process_rows(input_file_name, output_file_name):
    """Read from input file, convert temperature from Kelvin to Fahrenheit, and write to output file."""
